...
export default function AuthCodeVerification() {
  ...
  const [loginCode, { data: loginCodeData, isSuccess }] = useLoginCodeMutation()
  const [loginEmail, { data: loginMailData }] = useLoginEmailMutation()
  const borderColor = theme.palette.mode === ThemeMode.DARK ? theme.palette.secondary[200] : theme.palette.secondary.light;

  console.log(isSuccess,loginCodeData,"otpSuccess");
  
const resendCode = useCallback(()=>{
  loginEmail({email:email})
},[])
  useEffect(() => {
if(isSuccess && loginCodeData?.result?.accessToken){
  localStorage.setItem("access-token", loginCodeData?.result?.accessToken)
  navigate('/auth/register2', { replace: true });
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
        loginCode({ email: email, otp: values?.otp })
